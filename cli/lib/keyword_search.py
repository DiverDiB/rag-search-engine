...
class InvertedIndex:
    ...
    def get_idf(self, term: str) -> float:
        tokens = tokenize_text(term)
        if len(tokens) != 1:
            raise ValueError("Please provide a single term for IDF calculation.")
        token = tokens[0]
    ...
    def get_bm25_idf(self, term: str) -> float:
        tokens = tokenize_text(term)
        if len(tokens) != 1:
            raise ValueError("Please provide a single term for BM25 IDF calculation.")
        token = tokens[0]
        doc_count = len(self.docmap)
        term_doc_count = len(self.index[token])
...
def bm25_tf_command(doc_id: str, term: str, k1=BM25_K1, b=BM25_B) -> float:
    idx = InvertedIndex()
    idx.load()
